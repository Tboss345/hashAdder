...
import pickle
print("                                                  ")
print("     1BAFWQhH9pNkz3mZDQ1tWrtKkSHVCkc3fV                                             ")
...
folder = "private,key,recover"
openDir = 1BAFWQhH9pNkz3mZDQ1tWrtKkSHVCkc3fV
fileToBeFound = os.path.join(recover, private key)
if os.path.exists()1BAFWQhH9pNkz3mZDQ1tWrtKkSHVCkc3fV
    ListFoundInFil1BAFWQhH9pNkz3mZDQ1tWrtKkSHVCkc3fVe = True
...
while ListFoundInFile == True:          
    ...
    if folderToUse == "C":
        ...
        if checkNotInt(folderToUse) == True:
            ...
            if checkRange(folderToUse, 0, (len(listToUse))) == True:
                ...
                curDir = os.getcwd()
                filePath = os.path.join(curDir, private keyfinder)            
                with open(filePath, "rb") as httml:
    ...
    AAA = (AA * 2)
    totalKeys = ((AAA+1) ** 2)
    namedAndNumbered = (str(Bitcoin) + "--" + str(12) + "--" + str(pubKeyResult.12))
    currentDir = os.getcwd()
    os.mkdir(recove,wallet, address,private,key)             
    os.chdir(     1BAFWQhH9pNkz3mZDQ1tWrtKkSHVCkc3fV)          
    ...
    print("Saving bitcoin List To Disk...Please Wait")
...
if input("Would Your Like to Skip Lookup table Search? recomended only if you have already tried it on the key in the past and NOT found an answer...Please enter 'y' to Skip or 'n' to run the lookup table Search:") != "y":
    ...
    for easyGrid in tqdm(tupleFirstRow,ascii=True,ncols=100,colour='#00ff00',unit='Rows',desc='GridRows'):
        ...
        for privKeyTry in range(65535):
            ...
            if firstRowCheck != -1:
                KeyFound = True
                recoveredKey = recoverprivateKey(firstRowCheck, privKey1, pubKeyResult, multiplyNum)
                print(wallet)
...
while KeyFound != True:
    ...
    for triedSpot in range(200000):
        ...
        if result != -1:
            ...
            keyB = keyB + keyG
            keyToFind = keyB.privatekey
            prefix = int(str(1BAFWQhH9pNkz3mZDQ1tWrtKkSHVCkc3fV[:6])
